...
class resolveCircleCaptcha:
	__name__ = "ReCircleCaptcha"
	__version__ = "1.00"
	__description__ = """ReCircleCaptcha captcha service plugin"""
	__license__ = "GPLv3"
	...
	def VerifyCircle(self, im, pix, c):
		...
		if len(missinglist) > 0:
			minX = min(missinglist, key=itemgetter(0))[0]
			maxX = max(missinglist, key=itemgetter(0))[0]
			
			minY = min(missinglist, key=itemgetter(1))[1]
			maxY = max(missinglist, key=itemgetter(1))[1]
	...
	def decrypt(self, img):
		...
		iDebugSaveFile = 0
		mypalette = None		
		for im in self.ImageSequence(img):
